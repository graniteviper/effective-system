"""
SugarCRM data extractor implementation.
This module orchestrates the extraction of data from SugarCRM.
"""
...
from backend.extractors.connectors.sugarcrm_connector import SugarCRMConnector


class SugarCRMExtractor(BaseExtractor):
    """SugarCRM data extractor implementation."""

    def __init__(self, connector: SugarCRMConnector, storage: BaseStorageManager, config: Dict[str, Any]):
        super().__init__(connector, storage, config)
        self.config.setdefault('batch_size', 1000)
        self.config.setdefault('schema_extract', True)
        self.config.setdefault('extract_path', 'sugarcrm')
    ...
    def extract(self, object_names: List[str], extraction_type: str = 'full', since_date: Optional[str] = None) -> Dict[str, Any]:
        ...
        if not self.connector.validate_connection():
            ...
            results['errors'].append("Failed to connect to SugarCRM")
            return results
    ...
    def extract_incremental(self, object_name: str, since_date: str) -> Tuple[bool, Dict[str, Any]]:
        ...
        try:
            query_params = {
                ...
                'filter': f"date_modified > '{since_date}'"
            }
            ...
            success, store_path = self.storage.store_data(records, path)
            metadata.update({
                'success': success,
